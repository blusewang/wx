...
var qu struct {
	Str string  `param:"str" param:"str"`
	Int int64   `param:"int" param:"int"`
	Fl  float64 `param:"fl" param:"fl"`
}
var uv = url.Values{
	"str": []string{NewRandStr(32)},
	"int": []string{"234234"},
	"fl":  []string{"0.42342342"},
}

func BenchmarkMp_AppAuthToken(b *testing.B) {
	for i := 0; i < b.N; i++ {
		err := params.Unmarshal(uv, &qu)
		if err != nil {
			b.Error(err)
		}
	}
